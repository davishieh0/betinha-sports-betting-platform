...
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY', 'chave_padrao_temporaria')  # Chave de segurança para as sessões

# Funções de segurança para hash de senhas
...
def get_db():
    try:
        conexao = mysql.connector.connect(
            host=os.getenv('DB_HOST', '127.0.0.1'),         # IP do banco de dados.
            user=os.getenv('DB_USER', 'root'),              # Usuário de login do banco.
            password=os.getenv('DB_PASSWORD'),              # Senha do banco de dados.
            database=os.getenv('DB_NAME', 'bd_betinha'),    # Nome do banco de dados que estamos usando.
        )
        return conexao
...
mail_settings = {
    "MAIL_SERVER": os.getenv('MAIL_SERVER', 'smtp.gmail.com'),     # Servidor de e-mail (aqui, o Gmail).
    "MAIL_PORT": int(os.getenv('MAIL_PORT', '465')),               # Porta de envio do e-mail.
    "MAIL_USE_TLS": os.getenv('MAIL_USE_TLS', 'False').lower() == 'true',  # TLS desativado.
    "MAIL_USE_SSL": os.getenv('MAIL_USE_SSL', 'True').lower() == 'true',   # SSL ativado para segurança.
    "MAIL_USERNAME": os.getenv('MAIL_USERNAME'),                   # E-mail remetente.
    "MAIL_PASSWORD": os.getenv('MAIL_PASSWORD')                    # Senha do e-mail.
}
...
def register():
    if 'loggedin' in session:
        ...
        if account:  # Se já existe, informa o usuário.
            ...
            # Cria hash seguro da senha antes de armazenar
            hashed_password = hash_password(password)
            
            # Insere um novo usuário no banco de dados.
            # 'usuarios' tabela: armazena informações sobre os usuários, incluindo id, tipo de conta, nome, email, senha hasheada e data de nascimento.
...
def login():
    ...
    if request.method == 'POST':
        ...
        if connection:
            ...
            # Verifica se o usuário existe e se a senha está correta usando hash
